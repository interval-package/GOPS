"""
Here, we follow the instrucion of ppo.
The ppo has `val` and `ret` under the on policy setting.
The value is the value network approximated results, and the ret is the recursively calced res.

Here we using fra (feasible approximate), frt (feasible truth)
fra = value; frt = return

`ret` and `adv` are claced in the sampler (as additional indicator), so do the frt.

Assume they are claced in the sampler.

"""


__all__ = ["ApproxContainer", "FRPPO"]
...
class ApproxContainer(ApprBase):
    ...
    def __init__(self, **kwargs):
        ...
        self.frf: nn.Module = create_apprfunc(**value_args)

    def create_action_distributions(self, logits):
        return self.policy.get_act_dist(logits)


class FRPPO(AlgorithmBase):
    """
    ...
    def local_update(self, data: DataDict, iteration: int) -> dict:
        ...
        with torch.no_grad():
            ...
            data["fra"] = self.networks.frf(data["obs"])
    ...
    def _compute_loss(self, data: DataDict, iteration: int):
        ...
        mb_frt = frt.detach()
        mb_old_fra = fra
        mb_new_fra = self.networks.frf(mb_observation)

        # policy loss
        # Actually here is better to have batch notmalization
        ratio = torch.exp(mb_new_log_pro - mb_old_log_pro)
        sur1 = ratio * (mb_advantage + mb_frt)
        sur2 = ratio.clamp(1 - self.clip_now, 1 + self.clip_now) * (mb_advantage + mb_frt)
        ...
        # frf loss
        loss_frf = torch.pow(mb_new_fra - mb_frt, 2)
        ...
        loss_total = (
            ...
            + self.loss_coefficient_frf * loss_frf
